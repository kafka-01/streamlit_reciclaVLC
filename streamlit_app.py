...
def get_neighborhoods():
    url = "https://valencia.opendatasoft.com/api/records/1.0/search/?dataset=vulnerabilidad&q=&rows=-1"
    ...
    for neighborhood in neighborhoods_json:
        ...
        if "densitat_p" in neighborhood["fields"] and "geo_shape" in neighborhood["fields"]:

            # Get the GeoShape of the neighborhood
            geo_shape = shape(neighborhood["fields"]["geo_shape"])

            # Calculate the area of the neighborhood in km²
            area_neighborhood = geo_shape.area / 10**6  # Convert from m² to km²

            # Calculate the total population of the neighborhood
            population_density = float(neighborhood["fields"]["densitat_p"])

            population = population_density * area_neighborhood

        else:
            population = None
            population_density = None
            area_neighborhood = None

        neighborhoods.append({
            "name": ' '.join(word.capitalize() for word in neighborhood["fields"]["nombre"].split()),
            "geo_shape": neighborhood["fields"]["geo_shape"],
            "population": population,
            "neighborhood_area": area_neighborhood,
